...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/leagues/:leagueId/manual-points", async (req, res) => {
    ...
    try {
      ...
      // Check if user is member of the league
      const isMember = await storage.isUserInLeague(leagueId, userId);
      if (!isMember) {
        return res.status(400).json({ error: "L'utente non è membro di questa lega" });
      }

      await storage.setManualPoints(leagueId, userId, points);
  ...
  app.get("/api/leagues/:leagueId/manual-points", async (req, res) => {
    if (!req.session.userId) {
      return res.status(401).json({ error: "Non autenticato" });
    }
    ...
    try {
      ...
      const manualPoints = await storage.getAllManualPoints(leagueId);
      const result: Record<string, number> = {};
      
      for (const [userId, points] of manualPoints.entries()) {
